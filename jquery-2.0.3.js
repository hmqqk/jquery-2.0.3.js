 ...
 */
 //匿名函数，(function(){})()为匿名函数自执行，防止冲突。undefined我们在传参的时候可任意指定（它是一个属性）
...
var
	...
	//rootjQuery和$、jquery一样，只是查看到这里还没出现$/jquery的定义而已，rootjQuery=$(document)
	rootjQuery,
	...
	//获取字符串形式的undefined  typeof undefined="undefined"
	core_strundefined = typeof undefined,

	// Use the correct document accordingly with window argument (sandbox)
	//window下的变量存储，其实不存储我们也是直接拿过来就可以用的，但是就是有利于压缩的情况
	...
	//_jQuery和_$都是防止命名冲突,在jQuery初始化的时候保存了外部的$和jQuery
	...
	// [[Class]] -> type pairs
	//使用$.type()时候会用到,class2type={'[Object String]':'string','[Object Array]':'array'}
	class2type = {},

	// List of deleted data cache ids, so we can reuse them
	//以前版本和缓存数据是有关的，但是从现在开始已经没什么用处了，只是一个空数组
	core_deletedIds = [],
	...
	//局部的,window.jQuery = window.$ = jQuery;使其对外了
	jQuery = function( selector, context ) {
		
	
		// The jQuery object is actually just the init constructor 'enhanced'
		//（Jquery.fn.init是这里的构造函数），jQuery.fn就是jQuery的原型，则下面这个语句就是让我们去jquery.prototype下去找init方法
		//之所以这般处理主要是和传统的方法相比较，直接初始化（无需手动操作），对于调用后续的方法是非常便利的。
...
jQuery.fn = jQuery.prototype = {
  ...
  /*
	...
	*/
	constructor: jQuery,//修正constructor指向以免被修改
        ...
        // 3. match = ["<li>aaa","<li>",null]; $("<li>aaa")
			...
			if ( match && (match[1] || !context) ) {
				...
				if ( match[1] ) {//则说明是match=['<li>hello','<li>',null]  //$('<li>hello')这种情况 $("<li>")， $("<li>aaa")，$("<li>1</li><li>2</li>")
					...
					*/
					...
					//jQuery.parseHTML返回的是数组，经历过merge操作后返回的是json数据
					/*
					//parseHTML方法：第一个参数传入标签字符串"<li>"，第二个参数传入执行上下文document，第三个参数true代表可以新建script标签"<script><\/script>"（script前面的反斜杠需要转义，其它标签不需要），false代表不能。返回值是一个数组：$("<li>") ->   ["li"],  $("<li>1</li><li>2</li>") ->["li","li"]
          ...
          rsingleTag匹配单标签的，$("<li>")，$("<li></li>")
					*/
					...
					// HANDLE: $(html, props)（处理单标签，多标签的话是不能进入到这个分支的。例如：$('<li></li>',{title:'hi',html:'abcd'}).appendTo('ul');）
...
//这个是在火狐浏览器数据出错时执行的
	...
	nodeName: function( elem, name ) {//是否为指定节点名（内部方法）
		//为了能够全兼容，所以要转换成小写的，因为elem.nodeName测试时返回来的值是大写的
	...
	each: function( obj, callback, args ) {//遍历集合 args是用于jquery内部使用的
		...
		var value,
			i = 0,
			length = obj.length,
			//isArray就是判断是obj是类数组还是真正的数组，当然它不光针对它们，还针对jQuery对象this->{0:1,:,2:,length:3}所以是返回真的，所以这要和数组和json区分开来处理
	...
	grep: function( elems, callback, inv ) {//过滤新数组,函数目的是过滤出用户指定的数据。一般来说第三个参数不传或传为false，callback是一个过滤器，过滤成功返回true，过滤失败返回false。最终grep函数将callback返回成功的有elem元素返回。这个函数无论是在jQuery内部，或者我们自己使用都很有作用
		/*
		var arr=[1,2,3,4];
...
function Sizzle( selector, context, results, seed ) {
	...
	if ( documentIsHTML && !seed ) {
		...
		// Shortcuts//rquickExpr = /^(?:(<[\w\W]+>)[^>]*|#([\w-]*))$/;对于简单搜索快捷解析
		...
		// QSA pathh，如果能使用浏览器高级查询querySelectorAll
...
function createOptions( options ) {
	// 多个变量指向同一对象(或数组)引用时，其中一个变量修改了被引用对象的内部结构，其它引用变量也会表现出来
...
jQuery.support = (function( support ) {
	...
	if ( !input.type ) {//如今这段判断已无意义，因为所有浏览器的input.type都是有值的=text
		return support;
	}
...
function Data() {
	...
	//deineProperty所做的工作是这样的：
	/*
	例如定义了var obj={name:"hello"};
	如果调用了该函数后就变成了下面样子：
	var obj={name:"hello",0:{}}
	而且obj[0]在设定了之后是不能进行值得修改的
	*/
...
jQuery.extend({
	...
	queue: function( elem, type, data ) {
		...
		if ( elem ) {
			...
			if ( data ) {
				//如果elem还没有在cache中存储过名为type的数据，或者需要存储的数据是数组时也要重新创建（So当你加入队列中数组时会导致覆盖之间加入队列的元素）
...
jQuery.fn.extend({
    ...
    //生成的动画数据
        ...
        //之所以判断这个queue[0] !== "inprogress"是因为只要求第一次出队操作时要走jQuery.dequeue( this, type );（此时jQuery.dequeue( this, type )是不在的，当第二次出队时由于该项判断的不成立所以导致不能进入if语句完成下面的操作）
...
jQuery.fn.extend({
	//p.s. attr和prop不适合进行大量属性值的设置，这个用data比较好。attr和prop适合于设置元素本身的属性值或一些简单的数据
	//注意 attr和prop都会涉及Dom元素与对象之间相互引用，大部分浏览器就会出现内存泄露的问题，而data无论怎样都不用去担心内存泄露的问题。data的处理情况如图片5_1.png
	attr: function( name, value ) {//attr同js中的document.getElementById().setAttribute和getAttribute
	...
	prop: function( name, value ) {//prop同js中的document.getElementById()[]=''或者document.getElementById().name=''
